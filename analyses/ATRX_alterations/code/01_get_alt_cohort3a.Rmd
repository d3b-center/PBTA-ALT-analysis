...
manta_snv <- read_tsv(file.path(root_dir,
                                      "data",
...
```{r}
ATRX_sv <- manta_snv %>% 
  dplyr::filter(grepl("ATRX",Gene.name),FILTER=="PASS") 
...
if ( nrow(ATRX_sv) > 0){
  ...
  merge_ATRX <- merge_ATRX %>%
    full_join(ATRX_sv,by=c("Tumor_Sample_Barcode"="Kids.First.Biospecimen.ID.Tumor")) %>%
    dplyr::filter(Tumor_Sample_Barcode %in% cohort_plot_labels$Kids_First_Biospecimen_ID)
}
...
check_ratio %>% dplyr::filter(Hugo_Symbol == "ATRX"|!is.na(ATRX_DEL)|!is.na(ATRX_BND)|!is.na(ATRX_DUP)|!is.na(ATRX_INV)) %>% pull(ratio) %>% mean()
```


```{r}
# without ATRX alt in strelka2/mutect2/consensus 
check_ratio %>% dplyr::filter(is.na(Hugo_Symbol),is.na(ATRX_DEL),is.na(ATRX_BND),is.na(ATRX_DUP),is.na(ATRX_INV)) %>% pull(ratio) %>% mean()
```
