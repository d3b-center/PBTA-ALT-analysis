...
list_of_variates <- c("dmg_h3k28+group+telhunt_cat+reported_gender+atrx_mut",
...
for (ind_var in list_of_variates){
  ...
  # Save the table data in a TSV
  readr::write_tsv(table, file.path(output_dir, paste0("cox_reg_results_per_", ind_var, "_pfs_only_hgat.tsv")))
...
# Run without ATRX mutants

```{r}
meta_formatted_hgat_non_atrx <- meta_formatted_hgat %>%
  dplyr::filter(atrx_mut == "non_ATRX_mut")

for(ind_var in c("telhunt_cat", "phenotype")){
  # define model
  model <- paste0("survival::Surv(time = PFS_years, event = PFS_status) ~ ", ind_var)
  
  # depending on which variables are used, data used will be different
  data_used <- meta_formatted_hgat_non_atrx
  ...
  # save the output
  saveRDS(fit, file.path(output_dir, paste0("log_rank_survival_per_", ind_var, "_pfs_only_hgat_non_atrx.RDS")))
  ...
  # Save the plot
  cowplot::save_plot(filename = file.path(plots_dir, 
                                          paste0("logrank_survival_by_", ind_var, "_pfs_only_hgat_non_atrx.png")), 
                     plot = surv_plot_logrank)
