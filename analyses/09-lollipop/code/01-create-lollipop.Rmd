...
params:
  maf_in:
    ...
    input: file
  maf_atrx:
    label: "HGAT ATRX MAF file"
    value: output/hgat-atrx.maf.tsv
    input: file
  maf_out:
...
maf <- data.table::fread(file.path(root_dir, params$maf_in), select = keep_cols, data.table = F) %>%
  filter(Tumor_Sample_Barcode %in% hist$Kids_First_Biospecimen_ID_DNA,
         Hugo_Symbol == "ATRX") %>%
  unique() %>%
  write_tsv(file.path(analysis_dir, params$maf_atrx))
```

```{r annotate maf}
command <- paste(params$python_path, maf_annotator, '-i', file.path(analysis_dir, params$maf_atrx), '-o', file.path(analysis_dir, params$maf_out), '-b', oncokb_token, '-q hgvsp_short', '-r GRCh38')
command
